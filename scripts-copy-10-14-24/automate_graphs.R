...
side_by_side_hist <- function(segmented, field, descriptor, siteDescriptor, nbins){
	...
	m1 <- paste("Ground Truth Distribution of", descriptor,"at", siteDescriptor)
	m2 <- paste("Segmented Distribution of", descriptor, "at",siteDescriptor)
	fname <- paste0("resultgraphs/400/",descriptor,'_', siteDescriptor,".jpeg")
...
side_by_side_scatter <- function(segmentedH, fieldH, segmentedD, fieldD, siteDescriptor){
	...
	m1 <- paste("Ground Truth Crown Diameter vs. Crown Height at", siteDescriptor)
	m2 <- paste("Segmented Crown Diameter vs. Crown Height at", siteDescriptor)
	fname <- paste0("resultgraphs/400/scatterplot",'_', siteDescriptor,".jpeg")
	jpeg(fname,width = 1200, height = 800)
	par(mfrow=c(1,2))
...
filenames <- Sys.glob("las_outputs/400_las_outs/*.las")
...
for (plot in plotIDs){
	...
	fname <- paste0("las_outputs/400_las_outs/final_",plot,".las.las")
