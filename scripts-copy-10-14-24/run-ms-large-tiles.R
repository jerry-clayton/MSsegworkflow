...
#filenames <- Sys.glob("las_inputs/final_las_400/*.las")

filenames <- Sys.glob("las_inputs/sq_km_tiles_norm/*.las")
...
for (file in 26:length(filenames)) {
     #file <- filepath
	file <- filenames[file]
     ...
     tryCatch(
	      {
	ms_result <- MeanShiftR::parallel_MeanShift(point_clouds, lib.path = lib_path, frac.cores = 0.85, version = 'classic', H2CW = H2CW, H2CL = H2CD,minz=MINZ)

     # ms_result is a data.table
     print("Mean Shift segmentation done")
     ms_out <- paste0("1km_intermediates/ms_result_",strsplit(fname,'\\.')[[1]][1],".rds")
     print(ms_out)
     ...
     outfile <- paste0("sq_km_outs/", fname)
