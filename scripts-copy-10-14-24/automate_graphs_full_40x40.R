...
side_by_side_hist <- function(segmented, field, descriptor, siteDescriptor, nbins){
	...
	fname <- paste0("../resultgraphs/full/",descriptor,'_', siteDescriptor,".jpeg")
...
side_by_side_scatter <- function(segmentedH, fieldH, segmentedD, fieldD, siteDescriptor){
	...
	fname <- paste0("../resultgraphs/full/scatterplot",'_', siteDescriptor,".jpeg")
	jpeg(fname,width = 1200, height = 800)
	par(mfrow=c(1,2))
...
for (plot in plotIDs){
	...
	fname <- paste0("../las_outputs/norm_",plot,"_lidar_2021.las.las")
